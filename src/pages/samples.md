...
The following [code samples](https://github.com/AdobeDocs/express-add-on-samples/tree/main/script-runtime-samples) have been provided to help you get started using the [Document Sandbox and Adobe Express Document APIs](./references/authoring/index.md).

<InlineAlert slots="text" variant="warning"/>

**IMPORTANT:** The [Document Sandbox API's](./references/authoring/index.md) are currently **experimental only**. Please do not use them in any add-ons you plan to distribute or submit with updates until they have been deemed stable.
...
Demonstrates the use of the communication APIs to expose and proxy APIs bidirectionally between the iframe and document sandbox code environments. Also includes demonstrating how to use some of the [Web APIs](./references/authoring/web/) such as `setTimeout()` and `console.log()`.
