...
The sample add-on that the [Grid System tutorial](../learn/how-to/tutorials/grids-addon.md) is based upon. Demonstrates how to use the [Document APIs](../../references/document-sandbox/index.md) to create and manipulate shapes, context permanence, color pickers, and Spectrum Web Components usage.
...
The sample add-on that the [Document APIs Concepts](../learn/platform-concepts/document-api.md) article is based upon. It's used to discuss the Adobe Express Document Object Model, its Classes, Interface, and Constants. The add-on draws Dimensions (arrows measuring the width and height) around the selected shape.
