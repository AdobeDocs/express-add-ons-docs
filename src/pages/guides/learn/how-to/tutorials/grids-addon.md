...
faq:
  questions:
    - question: "What does this tutorial teach me to build?"
      answer: "A fully functional Grid System add-on that creates rows, columns, spacing (gutters), and color overlays."

    - question: "What's the difference between iframe and Document Model Sandbox?"
      answer: "iframe hosts the UI and logic; Document Model Sandbox allows direct document manipulation via the Document API."

    - question: "What are the prerequisites for this tutorial?"
      answer: "HTML/CSS/JavaScript knowledge, Adobe Express add-ons familiarity, Adobe Express account, and Node.js 20+."

    - question: "How do I get the project files?"
      answer: "Clone the GitHub repository or download ZIP from express-add-on-samples/document-sandbox-samples/express-grids-addon."

    - question: "What UI framework does this tutorial use?"
      answer: "Spectrum Web Components for consistent Adobe design system integration."

    - question: "How does communication work between iframe and sandbox?"
      answer: "Through the Communication API that connects the iframe UI with the Document Model Sandbox."

    - question: "What Document API concepts are covered?"
      answer: "Node insertion, creating/manipulating shapes, grouping elements, context permanence, and color management."

    - question: "Can I start from a blank template instead?"
      answer: "Yes, use the express-addon-document-api-template but manually add shapeUtils.js and Spectrum imports."
---
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon.png)
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon-animation.gif)
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon-communication.png)
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon-git.png)
...
This will install the required dependencies, build the project, and then serve it locally on port 5241; if you need more clarification about how to load an add-on in Adobe Express, please refer to the [quickstart](../../../getting-started/hello-world.md) guide for a step-by-step walkthrough.
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon-folder-structure.png)
...
![Add-on Communication API](src/pages/guides/learn/how-to/tutorials/images/grids-addon-communicationapi.png)
...
### 2.1 Using the Reference Documentation
...
![Add-on Communication API](src/pages/guides/learn/how-to/tutorials/images/grids-addon-reference.png)
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon-shape.png)
...
## 3. Coding the Grids add-on

### 3.1 Designing the UI with Spectrum Web Components
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon-swc.png)
...
// Import the ColorPicker components
import addOnUISdk, {
  ColorPickerEvent,
...
// Initialize color and store current value
let rowsColor = "#ccccff";
rowsColorSwatch.color = rowsColor;

// The <sp-swatch> click shows Adobe Express Color Picker
...
// Listen for color change events from the Color Picker
rowsColorSwatch.addEventListener(ColorPickerEvent.colorChange, function (event) {
  rowsColor = event.detail.color;
  rowsColorSwatch.setAttribute("color", rowsColor);
});

// Listen for color picker close events
rowsColorSwatch.addEventListener(ColorPickerEvent.close, function (event) {
  console.log("Color picker closed");
});
```

The `<sp-swatch>` click handler invokes Adobe Express's built-in Color Picker using `addOnUISdk.app.showColorPicker()`. When colors are selected, the `ColorPickerEvent.colorChange` event is fired, allowing us to update both the stored color value and the `<sp-swatch>` display. The Color Picker is positioned relative to the anchor element (the swatch) and can be customized with various options. Please note that color values are stored in variables for convenience—this allows easy access when creating the grid. Refer to the source code for other details on the HTML structure, which are not discussed here for brevity's sake.

### 3.2 Collecting values from the UI
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon-console.png)
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon-rowheight.png)
...
![](src/pages/guides/learn/how-to/tutorials/images/grids-addon-groups.png)
...
- **Spectrum Web Components** are crucial to UI building, but sometimes they require integration with platform APIs; in this project, we've linked a `<sp-swatch>` to Adobe Express's built-in Color Picker API to provide users with advanced color selection features like recommended swatches, themes, library access, and brand colors.

### 4.3 FAQs

#### Q: What does this tutorial teach me to build?

**A:** A fully functional Grid System add-on that creates rows, columns, spacing (gutters), and color overlays.

#### Q: What's the difference between iframe and Document Model Sandbox?

**A:** iframe hosts the UI and logic; Document Model Sandbox allows direct document manipulation via the Document API.

#### Q: What are the prerequisites for this tutorial?

**A:** HTML/CSS/JavaScript knowledge, Adobe Express add-ons familiarity, Adobe Express account, and Node.js 20+.

#### Q: How do I get the project files?

**A:** Clone the GitHub repository or download ZIP from express-add-on-samples/document-sandbox-samples/express-grids-addon.

#### Q: What UI framework does this tutorial use?

**A:** Spectrum Web Components for consistent Adobe design system integration.

#### Q: How does communication work between iframe and sandbox?

**A:** Through the Communication API that connects the iframe UI with the Document Model Sandbox.

#### Q: What Document API concepts are covered?

**A:** Node insertion, creating/manipulating shapes, grouping elements, context permanence, and color management.

#### Q: Can I start from a blank template instead?

**A:** Yes, use the express-addon-document-api-template but manually add shapeUtils.js and Spectrum imports.

### 4.4 Troubleshooting

This section covers common issues and solutions that may arise when building this add-on or similar Document API projects.

#### Communication API Issues

**Problem**: Methods can't call each other using `this` with arrow functions.
...
runtime.exposeApi({
  ...
  createShape: (shape) => {
    this.log("Shape created."); // Error: 'this' is undefined
  },
});
```

**Solution**: Use method shorthand syntax to provide proper `this` reference.
...
runtime.exposeApi({
  ...
  createShape(shape) {
    this.log("Shape created."); // Works correctly
  },
});
```
...
**Problem**: Interactive elements are enabled before SDK is ready, causing errors.

**Solution**: Always disable interactive elements initially and enable them only after SDK initialization.

```js
// ❌ Don't do this
<sp-button id="createGrid">Create</sp-button>
...
addOnUISdk.ready.then(async () => {
  // ... setup code ...
  createGridBtn.disabled = false; // Enable only when ready
});
```

#### Spectrum Web Components Import Errors

**Problem**: Import names don't match package names.

**Solution**: Note the difference between package installation and import paths.

```bash
# Install this
npm install @spectrum-web-components/button

# But import this (note the sp- prefix)
import "@spectrum-web-components/button/sp-button.js";
```

#### Validation and Error Handling

**Problem**: Users can input invalid values causing runtime errors.

**Solution**: Implement proper validation and avoid silent failures.

```js
// Set min/max values in HTML
<sp-number-field id="rows" min="1" max="20"></sp-number-field>

// Validate in Document API
addGrid({ columns, rows, gutter, columnColor, rowColor }) {
  if (rows < 1 || columns < 1 || gutter < 1) {
...
// Disable buttons when operations aren't available
deleteGridBtn.disabled = true; // Disable when no grid exists
```

#### Element Rendering Issues

**Problem**: Created elements don't appear in the document.

**Solution**: Always append elements to their container's `children` property.

```js
// ❌ This creates but doesn't render
const rect = editor.createRectangle();
rect.width = 200;
rect.height = 100;

// ✅ This creates AND renders
const rect = editor.createRectangle();
rect.width = 200;
rect.height = 100;
editor.context.insertionParent.children.append(rect); // Must append!
```

#### Color Handling Problems

**Problem**: Colors from UI don't work directly with Document API.

**Solution**: Convert hex strings using `colorUtils.fromHex()`.
...
const fillColor = editor.makeColorFill(color);
rect.fill = fillColor;
```

#### Enum Value Errors

**Problem**: Hard-coding enum values breaks when API changes.

**Solution**: Use named constants from the `constants` object.

```js
// ❌ Don't hard-code enum values
group.blendMode = 3; // Fragile - could change

// ✅ Use named constants
group.blendMode = constants.BlendMode.multiply; // Safe
```

#### Reference Management Issues

**Problem**: Can't delete or modify elements created earlier.

**Solution**: Store references to nodes for later manipulation.

```js
let gridRef = null; // Store reference

function addGrid() {
  const gridGroup = editor.createGroup();
  // ... populate group ...
  gridRef = gridGroup; // Save for later
}
...
function deleteGrid() {
  if (gridRef) {
    try {
      ...
      return "Error: Grid could not be deleted";
    }
  }
}
```
...
addOnUISdk.ready.then(async () => {
  ...
  colsColorSwatch.addEventListener(ColorPickerEvent.colorChange, function (event) {
...
<Details slots="list" repeat="13" summary="Click here to see the Changelog " />
...
- The project now uses the Adobe Express native Color Picker instead of the hidden `<input>` element workaround.
