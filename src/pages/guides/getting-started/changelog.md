...
The [`link()`](../../references/addonsdk/app-document.md#link) method and the [`LinkOptions`](../../references/addonsdk/addonsdk-constants.md) parameter are now stable and no longer require the `experimentalApis` flag in your add-on's `manifest.json`.
...
**IMPORTANT:** `identity()` is currently **_experimental only_**. To call it during development, set the `experimentalApis` flag to `true` in the [`requirements`](../../references/manifest/index.md#requirements) section of the `manifest.json`. Submissions with that flag enabled will fail validation, so remove it before distributing your add-on.

### Deprecated

- [`currentUser.userId()`](../../references/addonsdk/app-current-user.md#userid) is soft-deprecated in favor of [`identity()`](../../references/addonsdk/app-current-user.md#identity) and is scheduled for removal on **November 15, 2026**.
...
- New [how-to guide](../learn/how-to/large-document-support.md) with recipes for [`visitPages`](../../references/document-sandbox/document-apis/classes/page-list.md#visitpages), [`keepContentActiveDuringAsync`](../../references/document-sandbox/document-apis/classes/editor.md#keepcontentactiveduringasync), replacing deprecated APIs (`queueAsyncEdit`, `PageNode.artboards`, `PageNode.allDescendants`, `PageNode.allTextContent`, `PageNode.cloneInPlace`), and avoiding stale node references.
...
- New [Deprecation Policy](../learn/platform-concepts/deprecation-policy.md) guide explaining the API lifecycle from the developer's and user's perspective.
...
- The Adobe Express Add-on MCP Server (Beta) has been replaced by the **[Adobe Express Developer MCP Server](./local-development/mcp-server.md)**, which is now officially supported and production ready.
...
- New [Known Issues & Limitations](./local-development/known-issues-limitations.md#chrome-local-network-access-restriction) documentation for Chrome Local Network Access restriction (Chrome 142+) that affects local add-on development
...
- Added informational alert in the [Browser debugging guide](./local-development/browser.md) to notify Chrome users about the Local Network Access permission requirement
...
- Added a new [licensed-addon code sample](../learn/samples.md#licensed-addon) to illustrate how to implement monetization by leveraging the current userid.
- Added [`devFlags` API](../../references/addonsdk/app-dev-flags.md) details, which can be used to simulate certain behavior during development.
