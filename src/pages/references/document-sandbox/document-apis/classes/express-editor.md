...
• `get` **context**(): [`ExpressContext`](express-context.md)
...
• **createThreadedText**(`parentNode`, `textContent`, `geometry`?): [`ThreadedTextNode`](threaded-text-node.md)
...
• **parentNode**: [`ContainerNode`](../interfaces/container-node.md)
