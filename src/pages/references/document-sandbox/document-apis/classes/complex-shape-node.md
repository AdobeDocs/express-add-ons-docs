...
node. Both nodes must share the same [visualRoot](complex-shape-node.md#visualroot), but can lie anywhere within that subtree
...
• **cloneInPlace**(): [`ComplexShapeNode`](complex-shape-node.md)
