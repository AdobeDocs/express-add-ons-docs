...
node. Both nodes must share the same [visualRoot](fillable-node.md#visualroot), but can lie anywhere within that subtree
...
• **cloneInPlace**(): [`FillableNode`](fillable-node.md)
...
Both nodes must share the same [visualRoot](fillable-node.md#visualroot), but can lie anywhere within that subtree relative to one
