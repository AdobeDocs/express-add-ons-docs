...
node. Both nodes must share the same [visualRoot](./ellipse-node.md#visualroot), but can lie anywhere within that subtree
...
• **cloneInPlace**(): [`EllipseNode`](./ellipse-node.md)
...
Both nodes must share the same [visualRoot](./ellipse-node.md#visualroot), but can lie anywhere within that subtree relative to one
